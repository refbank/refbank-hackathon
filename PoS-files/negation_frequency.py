...
# Negation rate per message
df["negation_rate"] = df["negation_count"] / df["word_count"]

# Plot with 95% CI
plt.figure(figsize=(10, 6))
sns.lineplot(data=df, x="rep_num", y="negation_rate", marker="o", ci=95)
plt.title(f"{paper_name}: Negation Rate per Word with 95% CI")
plt.xlabel("Repetition Number")
plt.ylabel("Negations per Word")
plt.grid(True)
plt.tight_layout()
plt.show()
