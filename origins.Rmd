...
earliest_describer_fuzzy <- earlier_concat |> filter(role=="describer") |> 
...
survival_describer_fuzzy <- earlier_concat |> filter(role=="describer") |> 
  group_by(game_id, target, paper_id, structure, group_size) |> 
...
earliest_describer_base |> mutate(first=ifelse(rep_num>6, "7+", rep_num |> as.character()))  |> 
...
```{r}
earliest_describer_fuzzy |> mutate(first=ifelse(rep_num>6, "7+", rep_num |> as.character()))  |> 
  ggplot(aes(x=group_size, group=rep_num, fill=as.factor(first)))+geom_bar(position="fill", stat="count")+facet_wrap(~structure)
