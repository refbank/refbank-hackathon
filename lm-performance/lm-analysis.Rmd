...
data_prop <- data_count %>% group_by(model, label) %>% mutate(prop = n / sum(n)) %>% filter(nchar(model_choice) == 1)
...
KL <- data_prop_full %>% group_by(model) %>% summarise(KL = sum(prop * log(prop /hpf_no_zero$prop)))
